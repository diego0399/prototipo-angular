...
export class DataService {
  ...
  cargar(): void {
    ...
    if (this.hidratarDesdeLocalStorage()) {
      ...
      if (this.catalogoSoftware().length === 0) {
        this.http.get<SoftwareCatalogo[]>('assets/data/catalogo-software.json')
          .subscribe((c) => this.catalogoSoftware.set(c));
      }
    ...
    forkJoin({
      ...
      this.catalogoSoftware.set(r.catalogoSoftware);
  ...
  private hidratarDesdeLocalStorage(): boolean {
    try {
      ...
      this.catalogoSoftware.set(d.catalogoSoftware ?? []);
      return true;
    } catch {
      return false;
    }
  ...
  /** Software del catálogo, activo, aplicable a un tipo de equipo y a un formulario. */
  softwareAplicable(tipo: 'Laptop' | 'Desktop', formulario: 'F0288' | 'F0302'): SoftwareCatalogo[] {
    return this.catalogoSoftware().filter((s) =>
      s.activo && (formulario === 'F0288' ? s.aplicaF0288 : s.aplicaF0302) &&
      (tipo === 'Desktop' ? s.aplicaCPU : s.aplicaLaptop));
  }
  ...
  private validarSoftwareCatalogo(datos: SoftwareCatalogo, codigoOriginal: string | null): string | null {
    ...
    if (!datos.aplicaCPU && !datos.aplicaLaptop) return 'El software debe aplicar al menos a CPU o a Laptop.';
    if (!datos.aplicaF0288 && !datos.aplicaF0302) return 'El software debe aplicar al menos a F0288 o a F0302.';
    return null;
  }

  /** Agrega un software nuevo al catálogo. Devuelve null si se guardó, o el mensaje de la validación que falló. */
  agregarSoftwareCatalogo(datos: SoftwareCatalogo, usuario: string): string | null {
    const limpio: SoftwareCatalogo = {
      ...datos, codigo: datos.codigo.trim(), nombre: datos.nombre.trim(), categoria: datos.categoria.trim(),
    ...
    this.registrarEvento('CATALOGO-SOFTWARE', usuario, `Software agregado al catálogo: ${limpio.codigo} — ${limpio.nombre}`,
      limpio.activo ? 'Activo' : 'Inactivo', limpio.observacion, false, { modulo: 'Catálogo de software' });
    return null;
  }
  ...
  cambiarEstadoSoftwareCatalogo(codigo: string, activo: boolean, usuario: string): void {
    this.catalogoSoftware.update((list) => list.map((s) => (s.codigo === codigo ? { ...s, activo } : s)));
  ...
  private plantillaF0288(unidad: 'Soporte' | 'Hardware', condicion: 'Nuevo' | 'Usado', tipo: 'Laptop' | 'Desktop'):
    ...
    secciones.push({
      ...
      items: [
        { ...item('Instalación de Windows'), codigoSoftware: 'SOFT-001' },
    ...
    if (unidad === 'Soporte') {
      secciones.push({
        ...
        items: [
          { ...item('Antivirus'), codigoSoftware: 'SOFT-003' },
          { ...item('OCS Inventory'), codigoSoftware: 'SOFT-004' },
  ...
  crearExpedienteUnico(id: string, tecnicoConfiguracion: string, usuario: string): ExpedienteUnico | null {
    ...
    if (!this.configuracionDe(id)) {
      ...
      // El software del checklist F0302 se arma desde el catálogo de software permitido, filtrado
      // por tipo de equipo (p. ej. el Cliente VPN solo aplica a laptops) y solo con software
      // «aplicaF0302». «Agente DLP» no está en el catálogo dado (es parte del flujo SISSOR/dominio
      // ya existente), así que se conserva como ítem libre, fuera del control de versiones.
      ...
      const software = [
        ...this.softwareAplicable(eq?.tipo ?? 'Desktop', 'F0302').map(swCatalogo),
